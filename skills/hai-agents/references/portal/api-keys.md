...
- **Deterministic naming + revoke-by-name**: names are unique per org, so use a stable, deterministic name per purpose (e.g. `ci-{repo}`, `agents-api-demos-local`). To rotate without orphans: GET the list, find entries with your name, DELETE them by `id`, then POST a fresh key with the same name. Creating first and deleting later fails with `api_key_name_already_exists`.
