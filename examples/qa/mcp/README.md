# `qa/mcp` — QA a web UI from Claude Code
...
result = run_session(
    ...
    agent=Agent(
        name="ui-reviewer",
        instructions=REVIEWER_INSTRUCTIONS,    # loaded from ../prompts/reviewer_instructions.md
        skills=load_agent_skills(),            # loaded from ../agent_skills/*.md
...
Skills are named instruction documents the agent can load on demand. They live next door at [`../agent_skills/`](../agent_skills/) (shared with `qa/cli`) as markdown files with YAML frontmatter (`name`, `description`, body):
...
> These are **not** Claude Code skills (top-level `skills/`). Agent skills travel with the remote agent session; the top-level `skills/` directory contains instructions for the local Claude Code assistant.

## Running standalone

The same logic is also wired to a plain CLI in [`../cli/`](../cli/):
